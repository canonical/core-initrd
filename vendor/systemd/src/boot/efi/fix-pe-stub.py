...
def fix_aarch64_pe_stub():
    with open("stub", "r+b") as stub:
        ...
        section_align = int.from_bytes(stub.read(4), 'little')
        file_align = int.from_bytes(stub.read(4), 'little')

        stub.seek(pe_off + sz_signature + sz_coff_fhead +
                  off_opth_NumberOfRvaAndSizes)
        num_dirs = int.from_bytes(stub.read(4), 'little')
        ...
        for s in range(num_sect):
            ...
            if sect_name == b'.data\x00\x00\x00':
                ...
                data_sz = int.from_bytes(stub.read(4), 'little')
                data_va = int.from_bytes(stub.read(4), 'little')
                disk_data += data_sz
                linux_va = data_va + align_to_size(data_sz, section_align)
